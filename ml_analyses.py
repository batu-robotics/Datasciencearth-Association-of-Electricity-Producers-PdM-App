...
class Data:
    ...
    def analyse_data(self):
        ...
        plt.figure('Öznitelik Matrisi-1')
        plt.title('Korelasyon Matrisi Öncesi Öznitelik Sayısı= {}'.format(self.independent_test.shape[1]))
    ...
    def features(self):
        self.correlation_matrix=self.independent_train.corr()
        self.mask=np.triu(np.ones_like(self.correlation_matrix, dtype=bool))
        self.df_mask_old=self.correlation_matrix.mask(self.mask)
        
        self.upper_triangle=self.correlation_matrix.where(np.triu(np.ones(self.correlation_matrix.shape),k=1).astype(np.bool))
        
        self.to_drop_features=[column for column in self.upper_triangle.columns if any(abs(self.upper_triangle[column]) > 0.75)]
        ...
        self.correlation_matrix_new=self.filtered_train.corr()
        self.mask_new=np.triu(np.ones_like(self.correlation_matrix_new, dtype=bool))
        self.df_mask_new=self.correlation_matrix_new.mask(self.mask_new)
        
        plt.figure('Öznitelik Matrisi-2')
        plt.title('Korelasyon Matrisi Sonrası Öznitelik Sayısı= {}'.format(self.filtered_test.shape[1]))
        sns.heatmap(self.filtered_train.corr(),annot=False)
    ...
    def main_program(self):
        ...
        return self.df_mask_old,self.df_mask_new,\
...
df_corr_old,df_corr_new,scores_1,scores_all=d.main_program()
